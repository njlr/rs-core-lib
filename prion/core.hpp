...
namespace Prion {
    ...
    T random_integer(RNG& rng, T a, T b) {
        // We need a type big enough for both the RNG and output ranges.
        PRI_STATIC_ASSERT(std::is_integral<T>::value);
        using R = typename RNG::result_type;
        using T2 = std::conditional_t<(sizeof(T) > sizeof(R)), T, R>;
        using T3 = std::conditional_t<(sizeof(T2) > sizeof(int)), T2, int>;
        using U = std::make_unsigned_t<T3>;
        static constexpr auto zero = U(0), one = U(1);
        // Compare the input range (max-min of the values generated by the RNG)
        // with the output range (max-min of the possible results).
        if (a > b)
            std::swap(a, b);
        auto rng_range = U(rng.max() - rng.min());
        auto out_range = U(b - a);
        auto result = zero;
        if (out_range < rng_range) {
            // The RNG range is larger than the output range. Divide the output of
            // the RNG by the rounded down quotient of the ranges. If one range is
            // not an exact multiple of the other, this may yield a value too
            // large; try again.
            auto quotient = (rng_range - out_range) / (out_range + one) + one;
            do result = U(rng()) / quotient;
                while (result > out_range);
        } else if (out_range == rng_range) {
            // The trivial case where the two ranges are equal.
            result = U(rng() - rng.min());
        } else {
            // The output range is larger than the RNG range. Split the output
            // range into a quotient and remainder modulo the RNG range +1; call
            // this function recursively for the quotient, then call the RNG
            // directly for the remainder. Try again if the result is too large.
            auto quotient = (out_range - rng_range) / (rng_range + one) + one;
            auto high = zero, low = zero;
            do {
                high = random_integer(rng, zero, quotient - one) * (rng_range + one);
                low = U(rng() - rng.min());
